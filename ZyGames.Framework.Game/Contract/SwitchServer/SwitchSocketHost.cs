...
namespace ZyGames.Framework.Game.Contract.SwitchServer
{
    ...
    /// 路由服Socket通讯宿主基类
    /// </summary>
    public abstract class SwitchSocketHost : GameBaseHost
    {
        ...
        /// 跳转到大厅服
        /// </summary>
        private async System.Threading.Tasks.Task JumpToLobbyServer(GameSession lobbySs, ConnectionEventArgs e)
        {
            var data = new byte[e.Data.Length];
            Buffer.BlockCopy(e.Data, 0, data, 0, e.Data.Length);
            await lobbySs.SendAsync(data, 0, data.Length);
        }
