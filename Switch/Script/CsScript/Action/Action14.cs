...
namespace Switch.Script.CsScript.Action
{
    ...
    public class Action14 : BaseAction
    {
        ...
        public Action14(ActionGetter actionGetter)
            : base(ActionIDDefine.CstAction14, actionGetter)
        {
        ...
        public override bool TakeAction()
        {
            ...
            if (key == _word && !ServerHelper.IsExistLS())
            {
                Current.Bind(new SessionUser() { UserId = (int)GlobalDefine.ServerIdType.LobbyId, PassportId = string.Format("LobbyS-{0}", _serverId) });
                //设置忽略锁
                Current.SetIgoreLock(true, actionId);
                ServerHelper.SetLS(_serverId, Current);
                _packet.Ret = 1;
            }
            else
            {
        ...
        public async void SendLoginResult()
        {
            ...
            param["ActionId"] = ActionIDDefine.CstAction15;
            param["msgid"] = 0;
